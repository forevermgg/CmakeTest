...
bool JSONObject::SetObject(const std::string &key, JSONValue::SP value) {
  if (key.empty() || nullptr == value.get()) return false;
...
JSONValue::SP JSONObject::GetObject(const std::string &key) const {
  ...
  if (iter == end) return JSONValue::SP();
  return iter->second;
}
...
bool JSONObject::GetObjectAsString(const std::string &key,
  ...
  if (!JSONString::classof(value_sp.get())) return false;
...
bool JSONArray::SetObject(Index i, JSONValue::SP value) {
  if (value.get() == nullptr) return false;
...
JSONValue::SP JSONArray::GetObject(Index i) {
  if (i < m_elements.size()) return m_elements[i];
  return JSONValue::SP();
}
...
JSONParser::Token JSONParser::GetToken(std::string &value) {
  ...
  switch (ch) {
    ...
    case 't':
      if (GetChar() == 'r')
        if (GetChar() == 'u')
          if (GetChar() == 'e') return Token::True;
      break;
    ...
    case 'f':
      if (GetChar() == 'a')
        if (GetChar() == 'l')
          if (GetChar() == 's')
            if (GetChar() == 'e') return Token::False;
      break;

    case 'n':
      if (GetChar() == 'u')
        if (GetChar() == 'l')
          if (GetChar() == 'l') return Token::Null;
      break;
        ...
        switch (next_ch) {
          ...
          case '.':
            if (got_decimal_point) {
              error << "error: extra decimal point found at offset "
                    << start_index;
              value = error.str();
              return Token::Status;
            } else {
          ...
          case '-':
            ...
            if (exp_index == m_index - 1) {
              ...
              error << "error: unexpected " << next_ch
                    << " character at offset " << start_index;
              value = error.str();
              return Token::Status;
            }
            break;
        ...
        if (got_decimal_point) {
          if (exp_index != 0) {
            ...
            if (got_frac_digits) {
              ...
              error << "error: no digits after decimal point \""
                    << value.c_str() << "\"";
              value = error.str();
              return Token::Status;
            }
  ...
  if (ch == '\\') {
    ...
    switch (ch2) {
      ...
      case 'u': {
        ...
        if (hi_byte >= 0 && lo_byte >= 0) return hi_byte << 8 | lo_byte;
        return -1;
      } break;
    }
    return ch2;
...
JSONValue::SP JSONParser::ParseJSONValue(const std::string &value,
  ...
  switch (token) {
    ...
    case JSONParser::Token::Integer: {
      if (value.front() == '-') {
        bool success = false;
        int64_t sval = base::StringConvert::ToSInt64(value.c_str(), 0, 0, &success);
        if (success) return JSONValue::SP(new JSONNumber(sval));
      } else {
        bool success = false;
        uint64_t uval = base::StringConvert::ToUInt64(value.c_str(), 0, 0, &success);
        if (success) return JSONValue::SP(new JSONNumber(uval));
      }
    } break;

    case JSONParser::Token::Float: {
      bool success = false;
      double val = base::StringConvert::ToDouble(value.c_str(), 0.0, &success);
      if (success) return JSONValue::SP(new JSONNumber(val));
    } break;
