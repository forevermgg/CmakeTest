...
absl::Status testNormalWithAbortCheckButNoAbort() {
    ...
    InterruptibleRunner interruptibleRunner(should_abort,
...
Java_com_mgg_base_NativeLib_stringFromJNI(
    ...
    absl::StatusOr<std::unique_ptr<HttpRequest>> request =
            client::http::InMemoryHttpRequest::Create("https://valid.com",
                                        HttpRequest::Method::kGet, {}, "",
                    /*use_compression=*/false);
    auto interruptible_runner = std::make_unique<InterruptibleRunner>(should_abort_protocol_callback, timing_config);
    ...
    absl::StatusOr<std::vector<absl::StatusOr<client::http::InMemoryHttpResponse>>> results =
            client::http::PerformMultipleRequestsInMemory(
