...
StdStringExtractor::StdStringExtractor(const char *packet_cstr)
    : m_packet(), m_index(0) {
  if (packet_cstr) m_packet.assign(packet_cstr);
}
...
bool StdStringExtractor::GetHexU8Ex(uint8_t &ch, bool set_eof_on_fail) {
  ...
  if (byte == -1) {
    if (set_eof_on_fail || m_index >= m_packet.size()) m_index = UINT64_MAX;
...
size_t StdStringExtractor::GetHexBytes(void *dst_void, size_t dst_len,
  ...
  for (size_t i = bytes_extracted; i < dst_len; ++i) dst[i] = fail_fill_value;
...
size_t StdStringExtractor::GetHexByteString(std::string &str) {
  ...
  char ch;
  while ((ch = GetHexU8()) != '\0') str.append(1, ch);
  return str.size();
}
...
size_t StdStringExtractor::GetHexByteStringTerminatedBy(std::string &str,
  ...
  str.clear();
  char ch;
  while ((ch = GetHexU8(0, false)) != '\0') str.append(1, ch);
  if (Peek() && *Peek() == terminator) return str.size();
...
void StdStringExtractor::SkipSpaces() {
  ...
  while (m_index < n && isspace(m_packet[m_index])) ++m_index;
}
