...
#ifndef BASE_UTIL_FUNCTION_REF_HPP
#define BASE_UTIL_FUNCTION_REF_HPP
...
///
/// This type is similar to std::function, but unlike std::function holds a
/// reference to the callable rather than making a copy of it. This means that
/// it will never require a heap allocation, and produces significantly smaller
/// binaries due to the template machinery being much simpler. This type should
/// only ever be used as a function parameter that is not stored past when the
/// function returns. All other uses (including trying to store it in a
/// std::function) are very unlikely to be correct.
///
/// This implements a subset of P0792R5, which hopefully will be incorportated
/// into a future version of the standard library.
template <typename Signature>
class FunctionRef;
...
class FunctionRef<Return(Args...)> {
 public:
...
#else
#ifdef _WIN32
  // VC++ incorrectly rejects multiple versions of a defaulted special member
  // function
  constexpr FunctionRef(ThisType& o) noexcept
      : m_obj(o.m_obj), m_callback(o.m_callback) {}
  ...
  constexpr FunctionRef(F&& f) noexcept
      : m_obj(const_cast<void*>(
            reinterpret_cast<const void*>(std::addressof(f)))),
        m_callback([](void* obj, Args... args) -> Return {
          return (*reinterpret_cast<typename std::add_pointer<F>::type>(obj))(
